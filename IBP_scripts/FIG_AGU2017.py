...
smpath = root_path + 'Data From SMP\\'
...
def stackplot():
    ...
    rp = rp.resample('10Min',on='date_time').sum().reset_index()
    rp_IBP = rp.copy()
    rp_IBP.loc[rp_IBP.flag == 0,"precip_cm"] = np.nan
    rp_CW = rp.copy()
    rp_CW.loc[rp_CW.flag == 1,"precip_cm"] = np.nan
...
def transplot():
    title = "Transect; Sand ridge to pond"
    xloc_dict = {
            'WLW7':	0.0,
            'WLW8':	130.501,
            'WLW9':	218.29,
