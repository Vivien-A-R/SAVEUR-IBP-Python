...
#Use the above function to calculate the fraction of the timeseries that the sample is wet
# =============================================================================
# toggle_f = sect_rec.copy().drop(['section_recovery_cm','section_length_cm','factor'],axis = 1)
# toggle_f['toggle_period_seconds']  = toggle_f.apply(lambda x : toggle_count(x['well'],x['top_elev_m'],par ='p'),axis =1)
# toggle_f = toggle_f[~toggle_f.well.str.contains("12")]
# toggle_f = toggle_f[~toggle_f.well.str.contains("14")]
# 
# toggle_f.to_csv("C:\Users\Vivien\Dropbox\IBP_Python\\scripts_and_filenames\\data_products\\toggle_per.csv", index = False)
# 
# 
# #Plot test
# for well in toggle_f.well.unique():
#     sub = toggle_f[toggle_f['well'] == well]
#     print sub
#     sub.plot(x = 'cum_depth_cm',y = 'toggle_period_seconds',title = well)
# 
# =============================================================================
#Inundation; REALLY computation-intensive

#Use the above function to calculate the fraction of the timeseries that the sample is wet
# =============================================================================
# oneyear_f = sect_rec.copy().drop(['section_recovery_cm','section_length_cm','factor'],axis = 1)
# oneyear_f['inundation_fraction'] = oneyear_f.apply(lambda x : fraction_wet2(x['well'],x['top_elev_m']),axis =1)
# oneyear_f['dry_fraction'] = oneyear_f.apply(lambda x : fraction_dry2(x['well'],x['top_elev_m']),axis =1)
# #Print results to screen and save csv file
# print oneyear_f
# oneyear_f.to_csv(data_path+"data_products\\inundation_1year.csv",sep=',',index=False)
# 
# =============================================================================
oneyear_f = sect_rec.copy().drop(['section_recovery_cm','section_length_cm','factor'],axis = 1)
oneyear_f['inundation_fraction'] = oneyear_f.apply(lambda x : fraction_wet(x['well'],x['top_elev_m']),axis =1)
#oneyear_f['dry_fraction'] = oneyear_f.apply(lambda x : fraction_dry2(x['well'],x['top_elev_m']),axis =1)
#Print results to screen and save csv file
print oneyear_f
trunc = oneyear_f[oneyear_f['cum_depth_cm'] <= 90].copy()
dataproduct_path = root_path + "IBP Project\Documents\Water Level Derived Products\\"
trunc.to_csv(dataproduct_path+"inundation.csv",sep=',',index=False)

gb = trunc.groupby(by = ['well'])
fig, ax = plt.subplots(figsize=(15,7))
gb.plot(x = 'inundation_fraction',y = 'top_elev_m',ax = ax)