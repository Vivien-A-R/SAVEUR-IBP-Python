...
def plot_raw(sensor_id = "all"):
    if(sensor_id == "all"):
        ...
        for sensor_id in sensor_list:
            ...
            x_axis_length = max_date-min_date
            ind_meta = sensor_meta[sensor_meta.data_id == sensor_id].reset_index(drop=True) #metadata for only individual sensor (likely 1-2 rows)
            num_heights = len(ind_meta)
            if num_heights > 1:
                for row in range(0,num_heights):
                    bottomelv = (ind_meta['top_elev_ft'][row]-ind_meta['cable_length_ft'][row])*0.3048
                    gelv = ind_meta['ground_elev_ft'][row]*0.3048
                    if row == 0:
                        startdate = min_date
                        enddate = ind_meta['date_change'][row + 1]
                    elif (row < (num_heights-2)):
                        startdate = ind_meta['date_change'][row]
                        enddate = ind_meta['date_change'][row + 1]
                    else:
                        startdate = ind_meta['date_change'][row]
                        enddate = max_date
                        
                    min_x = (startdate - min_date)/x_axis_length
                    max_x = (enddate - min_date)/x_axis_length
                    plt.axhline(y = bottomelv,c = 'r',xmin = min_x, xmax = max_x)
                    plt.axhline(y = gelv,c = 'g')

            else:
                bottomelv = (ind_meta['top_elev_ft'].item()-ind_meta['cable_length_ft'].item())*0.3048
                gelv = ind_meta['ground_elev_ft'].item()*0.3048
