...
raw_data_path = 'C:\Users\Packman-Field\Google Drive\Packman Group\Multifunctional Urban Green Spaces Research Project\IBP Project\Data from Field Tablet\SMP Data\\text files\\Salinity\\'
...
if(os.path.isfile(processed_data_path + 'datafiles_VIC.csv')==False):
    df_empty = pd.DataFrame({'files' : []})
    print "NO DATAFILE PROCESSING HISTORY FOUND: Creating datafiles_VIC.csv"
    df_empty.to_csv(processed_data_path + 'datafiles_VIC.csv',sep=',',columns = ['files'],index = False) ##saves new processed data file.

processed_files = pd.read_csv(processed_data_path+'datafiles_VIC.csv',names = ['files'])
...
for f in new_files.files:
    ...
    t = pd.read_table(p,sep=',',index_col=False,skiprows=c,
                      names = ['index','date_time','s2_raw','s2_VIC','null'],parse_dates=['date_time'])
    t = t.drop(['index','null','s2_raw'],1)
    t['qual_c'] = 1
    id = f[0:8]
...
processed_files.to_csv(processed_data_path + 'datafiles_VIC.csv',sep=',',index = False)

#Data load and plotting, for checking
y_vals = ['s2_VIC']
mp1 = pd.read_csv(processed_data_path + 'SMP1' + '_VIC_ibp_main.csv',parse_dates = ['date_time'])
mp2 = pd.read_csv(processed_data_path + 'SMP2' + '_VIC_ibp_main.csv',parse_dates = ['date_time'])
mp1.plot(x = 'date_time',y = y_vals,title = 'SMP1')
mp2.plot(x = 'date_time',y = y_vals,title = 'SMP2')
