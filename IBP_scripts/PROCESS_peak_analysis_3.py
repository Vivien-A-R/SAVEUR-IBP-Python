...
def peakfind(sensor_id, fs_suffix = 'f5_lowess', peak_thresh = 0.01):
...
def peakstats2(sensor_id, tup, fs_suffix):
    dtable = []
    ftable = []
    
    #sensor ID should come from tup
    df_w = pd.read_csv(paperII_path + "\\waterlevels\\" + sensor_id + '_' + fs_suffix + '.csv' , parse_dates=['date_time'])
...
#todo: write working call statement for 1 well, then multiple wells!
#TODO: Pull out plot statement and make its own function
df_p,df_f = peakstats2(peak_thresh = pt)
